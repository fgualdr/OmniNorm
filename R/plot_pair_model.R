...
plot_pair_model <- function(ll){
  ...
  #par(mfrow = c(1, 3), pty="s")
    ...
    for( j in 1 : n_pop){
      dens<- mixsmsn:::dSN(xx, model$mu[j], model$sigma2[j], model$shape[j])
      ...
      lines(segments( means[j], 0, means[j], model$pii[j] * mixsmsn:::dSN(model$means[j], model$mu[j], model$sigma2[j], model$shape[j]), lty = 2, lwd = 1, col = myColors[j]))
