...
RunNorm <- function(  mat_path,
                      ...
                      Save_results=TRUE,
                      BiocParam=NULL){
  if(is.numeric(n_pop_reference)){if(n_pop_reference > 3){stop("n_pop must be between 1 and 3")}}
  if(is.numeric(n_pop)){if(n_pop > 3){stop("n_pop must be between 1 and 3")}}
  # Import the tables
  cat("|| Import matrix\n")
  ...
  if(is.character(design_path)) {design = read.delim(design_path,sep="\t",stringsAsFactors=FALSE)}else{design=design_path}
  ...
  if(!is.null(fix_reference)){
    ...
    if(length(wrev) != length(fix_reference)){
      if(fix_reference == "random"){
        ...
        if(tot_samp<=50){fix_reference = design$Sample_ID}
